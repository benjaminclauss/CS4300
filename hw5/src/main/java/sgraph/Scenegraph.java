...
public class Scenegraph<VertexType extends IVertexData> implements IScenegraph<VertexType>
{
    ...
    public void animate(float time) {
        // Get drone propellers.
        INode propeller1 = nodes.get("propeller-1");
        INode propeller2 = nodes.get("propeller-2");
        INode propeller3 = nodes.get("propeller-3");
        INode propeller4 = nodes.get("propeller-4");

        // Rotate propellers.
        propeller1.setAnimationTransform(new Matrix4f().rotateY((float) Math.toRadians(time % 360)));
        propeller2.setAnimationTransform(new Matrix4f().rotateY((float) Math.toRadians(time % 360)));
        propeller3.setAnimationTransform(new Matrix4f().rotateY((float) Math.toRadians(time % 360)));
        propeller4.setAnimationTransform(new Matrix4f().rotateY((float) Math.toRadians(time % 360)));
    }
