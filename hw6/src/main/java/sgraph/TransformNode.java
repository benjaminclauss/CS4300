...
public class TransformNode extends AbstractNode {
    ...
    @Override
    public List<Light> getLightsForNodeAndChildren(Stack<Matrix4f> modelView) {
        List<Light> lights = this.getLightsForNode(modelView);

        modelView.push(new Matrix4f(modelView.peek()));
        modelView.peek().mul(animation_transform).mul(transform);
        if (child != null) {
            lights.addAll(child.getLightsForNodeAndChildren(modelView));
        }
        modelView.pop();
