...
public class Scenegraph<VertexType extends IVertexData> implements IScenegraph<VertexType> {
    ...
    public List<Light> getLights(Stack<Matrix4f> modelView) {
        return root.getLightsForNodeAndChildren(modelView);
    }
    ...
    public void draw(Stack<Matrix4f> modelView) {
        if ((root != null) && (renderer != null)) {
            List<Light> lights = this.getLights(modelView);
            renderer.light(lights);
    ...
    @Override
    public void animate(Map<String, Matrix4f> animationTransformations) {
        for (String nodeName : animationTransformations.keySet()) {
            INode node = nodes.get(nodeName);
            if (node == null) {
                continue;
            } else {
                Matrix4f animationTransformation = animationTransformations.get(nodeName);
                node.setAnimationTransform(animationTransformation);
            }
        }
    }
