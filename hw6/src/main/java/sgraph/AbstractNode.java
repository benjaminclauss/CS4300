...
public abstract class AbstractNode implements INode
{
    ...
    /**
     * The lights of this node with respect to the node's coordinate system
     */
    ...
    @Override
    public List<Light> getLightsForNode(Stack<Matrix4f> modelView) {
        List<Light> transformedLights = new ArrayList<>();

        for (Light light : this.lights) {
            Light transformedLight = new Light(light);
            // Transform light position (and spotlight if applicable).
            Vector4f lightPosition = light.getPosition();
            lightPosition = modelView.peek().transform(lightPosition);
            Vector4f spotDirection = light.getSpotDirection();
            spotDirection = modelView.peek().transform(spotDirection);
            transformedLight.setPosition(lightPosition);
            transformedLight.setSpotDirection(spotDirection.x, spotDirection.y, spotDirection.z);
            transformedLights.add(transformedLight);
        }
