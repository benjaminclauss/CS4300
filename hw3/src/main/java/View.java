...
public class View {
  ...
  public View(Vector3f cameraPosition, Vector3f cameraLookAt, Vector3f cameraUp) {
    ...
    this.cameraRadius = (float) Math.sqrt(Math.pow(cameraPosition.x, 2) + Math.pow(cameraPosition.z, 2));
  }
  ...
  public void draw(GLAutoDrawable gla) {
    ...
    if (turntableOn) {
      turnDegree = (0.01f + turnDegree) % 360;
      modelView.peek().lookAt(
              new Vector3f(
                      cameraRadius * (float) Math.sin(turnDegree),
                      cameraPosition.y,
                      cameraRadius * (float) Math.cos(turnDegree)),
              cameraLookAt,
              cameraUp)
              .mul(trackballTransform);
    } else {
      modelView.peek()
              .lookAt(cameraPosition, cameraLookAt, cameraUp)
              .mul(trackballTransform);;
    }
